...
timeseriesServer <- function(id,
  ...
  shiny::moduleServer(id,
                      function(input, output, session) {
                        ...
                        brush_range <-
                          shiny::reactiveValues(x = NULL, y = NULL, brush = NULL)
                        ...
                        output$Timeseries <- shiny::renderPlot({
                          ...
                          axis_lab_1 <- paste0(param_to_expression(param_rct()),
                                               '~`in`~',
                          ...
                          if (is.null(brush_range$x)) {
                            shiny::observe({
                              brush_range$x <-
                                c(min(data_param1_rct()[[x]],
                                      ifelse(!is.null(data_param2_rct()), data_param2_rct()[[x]], NA),
                                      na.rm=TRUE),
                                  max(data_param1_rct()[[x]],
                                      ifelse(!is.null(data_param2_rct()), data_param2_rct()[[x]], NA),
                                      na.rm=TRUE))
                              brush_range$y <-
                                c(min(data_param1_rct()[[y]],
                                      na.rm=TRUE),
                                  max(data_param1_rct()[[y]],
                                      na.rm=TRUE))
                            })
                          }

                          if (!is.null(sec_param_rct())) {
                            shiny::req(nrow(data_param2_rct()) > 0)

                            axis_lab_2 <- paste0(param_to_expression(sec_param_rct()),
                                                '~`in`~',
                            ...
                            #param_label_2 <- param_to_expression(sec_param_rct())
